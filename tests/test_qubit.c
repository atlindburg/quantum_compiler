...
void test_qubit_initialization(void) {
    ...
    TEST_ASSERT_EQUAL_DOUBLE_C(1, creal(q.state0));
    TEST_ASSERT_EQUAL_DOUBLE_C(0, cimag(q.state0));
    TEST_ASSERT_EQUAL_DOUBLE_C(0, creal(q.state1));
    TEST_ASSERT_EQUAL_DOUBLE_C(0, cimag(q.state1));
}
...
void test_apply_x_gate(void) {
    ...
    TEST_ASSERT_EQUAL_DOUBLE_C(0, creal(q.state0));
    TEST_ASSERT_EQUAL_DOUBLE_C(0, cimag(q.state0));
    TEST_ASSERT_EQUAL_DOUBLE_C(1, creal(q.state1));
    TEST_ASSERT_EQUAL_DOUBLE_C(0, cimag(q.state1));
}
